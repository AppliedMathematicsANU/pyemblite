...
from . cimport rtcore_buffer4 as rtcb
from . cimport rtcore_ray4 as rtcr
from . cimport rtcore_scene4 as rtcs
from . cimport rtcore_geometry4 as rtcg
from .rtcore4 cimport Vertex, Triangle
...
cdef class EmbreeScene:
    ...
    def run(self, np.ndarray[np.float32_t, ndim=2] vec_origins,
        ...
        cdef RTCIntersectArguments ray_args
        rtcInitIntersectArguments( &ray_args)
        
        cdef RTCOccludedArguments occl_args
        rtcInitOccludedArguments( &occl_args)
        ...
        for i in range(nv):
            ...
            if (query_type == intersect) or (query_type == distance):
                rtcIntersect1(self.scene_i, &ray_hit, &ray_args)
                ...
                rtcOccluded1(self.scene_i, &(ray_hit.ray), &occl_args)
